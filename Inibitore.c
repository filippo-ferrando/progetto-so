...
//I SIGUSR fanno riferimento allo stato dell'inibitore. SIGUSR1: Inibitore acceso, da 0 a Attivatore per non farlo partire
//SIGUSR2: Inibitore spento, da 1 ad Attivatore per tenerlo fermo
...
int main(int argc, char* argv[]){
    ...
    msgid = msgget(KEY_INHIB,IPC_CREAT|0600);
    //Imposto tipo di messaggio per la coda di messaggi
    messaggio.mtype = 1; 

    reserveSem(sem_start, 0); //Sincronizzo l'inibitore con il resto dei processi tramite semaforo.
    sleep(1);
    while(1){ 
        ...
        //DA SCRIVERE QUI SOTTO IMPLEMENTAZIONE!
...
//Se si raggiungono blackout/meltdown, ferma l'attivatore
